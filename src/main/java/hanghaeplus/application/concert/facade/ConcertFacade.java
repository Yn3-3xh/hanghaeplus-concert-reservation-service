...
public class ConcertFacade {
    ...
    public ConcertResponse.ConcertQueuePosition getConcertQueuePosition(ConcertRequest.ConcertQueuePosition request) {
        ...
        int concertQueuePosition = queueTokenQueryService.getWaitingPosition(new QueueQuery.CreateTokenWaiting(request.tokenId(), queue.getId()));
    ...
    public void enrollConcertQueue(ConcertRequest.ConcertQueueEnrollment request) {
        Queue queue = queueQueryService.getQueue(new QueueQuery.Create(request.concertId()));
        queueTokenCommandService.enrollQueueTokenWaiting(new QueueCommand.CreateTokenWaiting(request.tokenId(), queue.getId()));
    }
